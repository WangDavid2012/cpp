...
namespace mySTL{
	...
	private:
		...
		public:
			...
			{//mySTL::set的pos以1开始计数,因此加1,以下都是如此
				mySTL::set(head,position+1,mySTL::read(tmp.head,tmp.position+1));
				return *this;
			}
			self& operator = (const bool& tmp)
			{
				assert(head != 0);
				mySTL::set(head,position + 1,tmp);
				return *this;
			}
			bool operator == (const bool& tmp) const 
			{
				assert(head != 0);
				return mySTL::read(head,position + 1) == tmp;
			}
			bool operator != (const bool& tmp) const
			{
				assert(head != 0);
			...
			{
				assert(head != 0);
				return mySTL::read(head,position + 1) == 
				mySTL::read(tmp.head,tmp.position + 1);
			}
			bool operator != (const self& tmp) const 
			{
				return !(*this == tmp);
			...
			{
				return os << mySTL::read(const_cast<UCHAR*>(tmp.head),tmp.position + 1);
			}
			friend std::istream& operator >> (std::istream& is,self& tmp)
			{ // 标准库无此功能,感觉应该加上，个人见解，呵呵！
				bool tmp_val = false;
				is>>tmp_val;
				mySTL::set(tmp.head,tmp.position + 1,tmp_val);
				return is;
			}
		};
	...
	private:   //内部使用的一些函数
		...
		{ // mySTL里的read以1开始计数，在这里转换下,以下同理可得
			return mySTL::read(ptr,pos+1);
		}
		void* set(void* ptr,size_type pos,bool val = true) const  // 从0开始
		{ //从逻辑上讲，不应该用const，为了通用性，加了个const
		  // 也可以另外写一个非const版本
			return mySTL::set(ptr,pos+1,val);
		}

...
void filefunction_test()
{
    ...
    ofstream   ofs(_filepath.c_str(), ios::out);
    if (!ofs.bad()) {
        ofs.close();
    }
...
void file_test3()
{
    ...
    u64 addr = 12345678;
    std::string ip("172.16.100.32");
    ofstream ofs("devicehandle.dat");
    ...
    int ret = remove("devicehandle.dat");
    cout << "The remove result is " << ret << endl;
...
void readwhiteiptable()
{
    ...
    std::vector<std::string>   m_whiteip_vector;
    std::string _filepath("iptable.conf");
    std::ifstream _in(_filepath.c_str());  //定义一个输入文件流  ifstream
    if (!_in.good()) {
        _in.close();
        return;
    ...
    string line;
	//读取_in 文件的每一行数据，放入字符line中
    while (getline(_in, line)) {   
        std::istringstream iss(line);
        std::string ip;
        iss >> ip;
        m_whiteip_vector.push_back(ip);
    }
    _in.close();
 ...
 * \time 2020/11/02
 ************************************************************/
// int ip_allow(struct sockaddr_in cliaddr)
// {
    //std::vector<std::string>   m_whiteip_vector;
    //std::string ip = inet_ntoa(cliaddr.sin_addr);
    //auto it = m_whiteip_vector.begin();
    //for (; it != m_whiteip_vector.end(); it++) {
    //    if (ip == *it)
    //        break;
    //}

    //if (it == m_whiteip_vector.end())
    //    return 0;
    //else
//         return 1;
// }